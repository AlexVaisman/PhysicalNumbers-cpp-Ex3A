...
int main() {
  ...
  if (signal == 0) {
    ...
    .CHECK_OUTPUT(meter+cm, "1.01[meter]")
    .CHECK_OUTPUT(meter+meter, "2[meter]")
    .CHECK_OUTPUT(meter+km, "1001[meter]")
    ...
    .CHECK_OUTPUT(min+sec, "1.01667[min]")
    .CHECK_OUTPUT(min+min, "2[min]")
    .CHECK_OUTPUT(min+hour, "61[min]")

    .CHECK_OUTPUT(hour+sec, "1.000278[hour]")
    .CHECK_OUTPUT(hour+min, "1.01667[hour]")
    .CHECK_OUTPUT(hour+hour, "2[hour]")

    /**checking gram , kilo , ton**/
    .CHECK_OUTPUT(gram+gram, "2[gram]")
    .CHECK_OUTPUT(gram+kilo, "1001[gram]")
    .CHECK_OUTPUT(gram+ton, "1000001[gram]")

    .CHECK_OUTPUT(kilo+gram, "1.001[kilo]")
    .CHECK_OUTPUT(kilo+kilo, "2[kilo]")
    .CHECK_OUTPUT(kilo+ton, "1001[kilo]")
    ...
    .CHECK_OUTPUT(meter, "2[meter]")
    .CHECK_OUTPUT(gram, "2[gram]")
    .CHECK_OUTPUT(kilo, "2[kilo]")
    ...
    .CHECK_OUTPUT((cm-=cm), "0[cm]")
    .CHECK_OUTPUT((km-=km), "0[km]")
    .CHECK_OUTPUT((meter-=meter), "0[meter]")
    .CHECK_OUTPUT((sec-=sec), "0[sec]")
    .CHECK_OUTPUT((min-=min), "0[min]")
    .CHECK_OUTPUT((hour-=hour), "0[hour]")
    .CHECK_OUTPUT((gram-=gram), "0[gram]")
    .CHECK_OUTPUT((kilo-=kilo), "0[kilo]")
    .CHECK_OUTPUT((ton-=ton), "0[ton]")

     // checking if change took place
    .CHECK_OUTPUT(cm, "0[cm]")
    .CHECK_OUTPUT(km, "0[km]")
    .CHECK_OUTPUT(meter, "0[meter]")
    .CHECK_OUTPUT(gram, "0[gram]")
    .CHECK_OUTPUT(kilo, "0[kilo]")
    ...
    .CHECK_OUTPUT(cm, "1[cm]")
    .CHECK_OUTPUT(km, "1[km]")
    .CHECK_OUTPUT(meter, "1[meter]")
    .CHECK_OUTPUT(gram, "1[gram]")
    .CHECK_OUTPUT(kilo, "1[kilo]")
    .CHECK_OUTPUT(ton, "1[ton]")
    .CHECK_OUTPUT(sec, "1[sec]")
    .CHECK_OUTPUT(min, "1[min]")
    .CHECK_OUTPUT(hour, "1[hour]")
